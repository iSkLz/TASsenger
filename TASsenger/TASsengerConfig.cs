...
namespace TASsenger
{
    public class TASsengerConfig
    {
        ...
        {
            TASsenger.Log("Loading settings.");
            try
            {
                using (UMFConfig cfg = new UMFConfig())
                {
                    ...
                    if (cfgVer != string.Empty && cfgVer != configVersion)
                    {
                        ...
                        TASsenger.Log("The config file was outdated and has been deleted. A new config will be generated.");
                    }
                    ...
                    TASsenger.Log("Finished UMF Settings.");

                    //Add your settings here

                    TASsenger.Log("Finished loading settings.");
                }
            }
            catch (Exception e)
            {
                TASsenger.Log("Error loading mod settings: " + e.Message + "(" + e.InnerException?.Message + ")");
            }
        }
    }
}